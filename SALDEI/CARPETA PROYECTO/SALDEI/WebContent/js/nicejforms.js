...
jQuery.NiceJForms = {
	options : {
		...
		imagesPath               : "http://usaldei.uca.edu.sv:8080/SALDEI/css/images/default/"
		//imagesPath               : "http://localhost:8080/SALDEI/css/images/default/"
	},
	...
	{	
		...
		var urlbuild= jQuery.NiceJForms.options.imagesPath+ "blank.gif";
//		$(testImg).attr("src", jQuery.NiceJForms.options.imagesPath + "blank.gif").attr("id", "imagineTest");
		$(testImg).attr("src", urlbuild);
...
replaceCheckboxes: function () 
		...
		jQuery.NiceJForms.selects.each(function(q){
			...
			$(jQuery.NiceJForms.selects[q]).children().each(function(w){
				...
				if(jQuery.NiceJForms.selects[q].options[w].selected) {
					var x = $(jQuery.NiceJForms.selects[q]).attr("id");
	...
	selectValor: function(selectFieldId,  selectNo, valor) {
		selectField = $('#' + selectFieldId);
		sFoptions = selectField.children();
		var linkNo = 0;
		selectField.children().each(function(k){
			var x=  $(sFoptions[k]).val();
			if (valor == x){
				linkNo = k;				
	...
	cambio: function(id) {
		jQuery.NiceJForms.buttons.each(function(i){
			if (jQuery.NiceJForms.buttons[i].id == id){
				var buttonRight= $(this).prev();
				var buttonLeft= $(this).next();
				//alert(buttonLeft);
			 	jQuery(buttonRight).remove();
				jQuery(buttonLeft).remove();
				$(this).removeClass("buttonSubmit");
				$(this).attr("disabled","disabled");
				$(this).addClass("buttonSubmitRed");				
				var buttonLeftRed = document.createElement('img');
				jQuery(buttonLeftRed).attr({src: jQuery.NiceJForms.options.imagesPath + "button_left_red.gif"}).addClass("buttonImgRed");
				$(this).before(buttonLeftRed);
				var buttonRight = document.createElement('img');
				jQuery(buttonRight).attr({src: jQuery.NiceJForms.options.imagesPath + "button_right_red.gif"}).addClass("buttonImgRed");
