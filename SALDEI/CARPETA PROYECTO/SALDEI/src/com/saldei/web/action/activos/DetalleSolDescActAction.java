 ...
 * XDoclet definition:
 * 
 * @struts.action path="/detalleSolDescActAction" name="detalleSolDescActForm"
 *                input="/html/activos/detalleSolDescAct.jsp" scope="request"
 * @struts.action-forward name="updateFail"
 *                        path="/html/activos/detalleSolDescAct.jsp"
 * @struts.action-forward name="back"
 *                        path="/detalleSolDescActAction.jsp?accion=Find"
 * @struts.action-forward name="success"
 *                        path="/html/activos/detalleSolDescAct.jsp?accion="
 */
public class DetalleSolDescActAction extends BaseAction {
	...
	public ActionForward delete(ActionMapping mapping, ActionForm form,
		...
		DetalleSolDescActForm detalleSolDescActForm = (DetalleSolDescActForm) form;
		ActDetSolicitudDAO actDetSolicitudDAO = new ActDetSolicitudDAO();

		try {
			detalleSolDescActForm.validate(mapping, request, errors);
			if (errors.isEmpty()) {
				actDetSolicitudDAO.delete(detalleSolDescActForm
						.getActDetSolicitud());
				HibernateSessionFactory.getSession().beginTransaction()
						.commit();
				errors.add("detsoldesca.mensaje.exito", new ActionError(
						"detsoldesca.mensaje.exito.delete"));
			} else {
				target = "updateFail";
				setFLAG_UPDATE(true);
			}
			...
			errors.add("detsoldesca.mensajeError.error", new ActionError(
					"detsoldesca.mensajeError.error.nodelete"));
	...
	public ActionForward insert(ActionMapping mapping, ActionForm form,
		...
		try {
			detalleSolDescActForm.validate(mapping, request, errors);
			if (errors.isEmpty()) {
				actDetSolicitudDAO.save(detalleSolDescActForm
						.getActDetSolicitud());
				HibernateSessionFactory.getSession().beginTransaction()
						.commit();
				errors.add("detsoldesca.mensaje.exito", new ActionError(
						"detsoldesca.mensaje.exito.insert"));
			} else {
				setFLAG_UPDATE(true);
			}
		} catch (Exception e) {
	...
	public ActionForward update(ActionMapping mapping, ActionForm form,
		...
		try {
			detalleSolDescActForm.validate(mapping, request, errors);
			if (errors.isEmpty()) {
				actDetSolicitudDAO.merge(detalleSolDescActForm
						.getActDetSolicitud());
				HibernateSessionFactory.getSession().beginTransaction()
						.commit();
				errors.add("detsoldesca.mensaje.exito", new ActionError(
						"detsoldesca.mensaje.exito.update"));
			} else {
				target = "updateFail";
				setFLAG_UPDATE(true);
			}
	...
	public ActionForward find(ActionMapping mapping, ActionForm form,
		...
		try {
			...
			// actSolicitud =
			// actSolicitudDAO.findById(detalleSolDescActForm.getActSolicitud().getId());
			data = actDetSolicitudDAO.descActfindSolDet(detalleSolDescActForm
					.getCodSolicitud().toString(), session.getAttribute("user")
					.toString());

			request.setAttribute("listaRecursosxSolicitud", data);

			if (!isFLAG_UPDATE()) {
				detalleSolDescActForm.reset(mapping, request);
			} else {
	...
	public ActionForward enviarSolicitud(ActionMapping mapping,
		...
		actSolicitud = actSolicitudDAO.findById(detalleSolDescActForm
				.getActSolicitud().getId());

		if (!actSolicitud.getActDetSolicituds().isEmpty()) {
			actSolicitud.setEstado("E");
			detalleSolDescActForm.setEstado("E");
			detalleSolDescActForm.setEstadoNombre("Enviada");
			try {
				msgText = "Buen dia.<br><br>"
						...
						+ " "
						+ user.getPrimerApe()
						+ " ha enviado una solicitud para traslado de activos a bodega<br>"
						+ "con codigo "
						...
						+ "<br>consulte la opcion de aprobacion de traslado de activos a bodega"
						+ " en el sistema SALDEI.<br><br>";

				if (detalleSolDescActForm.getComentario() != null
						&& !detalleSolDescActForm.getComentario().trim()
								.equals(""))
					msgText += "El usuario " + user.getPrimerNom() + " "
							+ user.getPrimerApe()
							+ " especifico la siguiente descripcion:<br>"
							+ "\"" + detalleSolDescActForm.getComentario()
							+ "\"";
				msgText += "<br>--------------------------------------------------------------------"
						+ "-------------------------------------------------------------------<br>"
				...
				email
						.sendEmail(
								"UCA-SALDEI: Solicitud de Traslado de Activos a Bodega",
								"saldei@uca.edu.sv", new String[] { usuario
										.getEmail() }, null, msgText, null);
				actSolicitudDAO.merge(actSolicitud);
				HibernateSessionFactory.getSession().beginTransaction()
						.commit();
				errors.add("detsoldesca.mensaje.exito", new ActionError(
						"detsoldesca.mensaje.exito.enviada"));
	...
	public ActionForward back(ActionMapping mapping, ActionForm form,
		...
		// form.reset(mapping,request);
		DetalleSolDescActForm a = (DetalleSolDescActForm) form;
		request
